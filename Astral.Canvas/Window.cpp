...
namespace AstralCanvas
{
	...
	Window::Window(IAllocator allocator, const char *name, i32 width, i32 height, bool resizeable, bool maximized, bool fullscreen, void *iconData, u32 iconWidth, u32 iconHeight, LGFXSwapchainPresentationMode presentMode)
	{
		...
		if (handle != NULL)
		{
			...
			this->isVSync = swapchainCreateInfo.presentationMode == LGFXSwapchainPresentationMode_Fifo;
			
			glfwGetFramebufferSize(handle, &this->frameBufferSize.X, &this->frameBufferSize.Y);
			swapchainCreateInfo.width = (u32)this->frameBufferSize.X;
			swapchainCreateInfo.height = (u32)this->frameBufferSize.Y;
